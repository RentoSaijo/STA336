...
GPT-5.4 Codex was used to create the \texttt{YAML} portion and some \texttt{LaTeX} code to format the text/equations nicely. Page formatting code was also provided by Derin Gezgin. In the setup chunk, helper functions such as \texttt{table\_latex()}, \texttt{predict.regsubsets()}, and \texttt{choose\_ncomp()} were defined so the model-fitting output can be reported clearly and reproducibly. See the original \texttt{RMD} file \href{https://github.com/RentoSaijo/STA336/blob/main/Classwork3.Rmd}{here} for more details.
...
\NormalTok{pcr\_m   }\OtherTok{\textless{}{-}} \FunctionTok{choose\_ncomp}\NormalTok{(pcr\_fit)}
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-6}PCR test-set performance.}
\centering
\begin{tabular}[t]{cc}
\toprule
Selected $M$ & Test MSE\\
...
PCR chooses \(\boxed{M = 17}\) principal components, and the resulting test MSE is \(\boxed{1{,}182{,}899}\). This exactly matches the least squares test error on this split. That agreement makes sense here because \texttt{College} has \(17\) predictors, so using all \(17\) principal components reproduces the full least squares fit.
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-7}Comparison of the four required methods.}
\centering
\begin{tabular}[t]{ccc}
\toprule
...
\begin{table}[H]
\centering
\caption{\label{tab:unnamed-chunk-8}Best subset extension on the same split.}
\centering
