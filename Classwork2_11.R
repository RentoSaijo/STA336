...
# ============================================================
# STAGE C: tune gamma (min_split_loss)
# Fix: depth=3, mcw=1, subsample=0.5, colsample=1.0
# ============================================================
...
gamma_grid <- c(0, 0.05, 0.1, 0.2, 0.5, 1, 2, 5)

gridC <- tibble::tibble(gamma = gamma_grid) |>
...
for (i in seq_len(nrow(gridC))) {
  ...
  gridC$test_mse[i]  <- out$test_mse
  gridC$best_iter[i] <- out$best_iter

  cat(sprintf("[%2d/%2d] gamma=%.2f | best_iter=%d | test MSE=%.5f",
              i, nrow(gridC), g, out$best_iter, out$test_mse))

  if (out$test_mse < best_mse_C) {
    best_mse_C <- out$test_mse
    best_cfg_C <- gridC[i, ]
    cat("  <-- NEW BEST")
  }
  cat("\n")
...
cat("\nStage C results:\n")
print(dplyr::arrange(gridC, test_mse))
cat("\nBest (Stage C):\n")
print(best_cfg_C)