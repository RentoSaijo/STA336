...
# ============================================================
# STAGE B: tune subsample x colsample_bytree
# Fix: max_depth=3, min_child_weight=1 (best from Stage A)
# ============================================================

best_md  <- 3
best_mcw <- 1

sub_grid <- c(0.5, 0.7, 0.8, 0.9, 1.0)
col_grid <- c(0.5, 0.7, 0.8, 0.9, 1.0)

gridB <- expand.grid(subsample = sub_grid,
...
set.seed(100)
for (i in seq_len(nrow(gridB))) {
  ss  <- gridB$subsample[i]
  cs  <- gridB$colsample_bytree[i]
  ...
  gridB$test_mse[i]  <- out$test_mse
  gridB$best_iter[i] <- out$best_iter

  cat(sprintf("[%2d/%2d] subsample=%.1f | colsample=%.1f | best_iter=%d | test MSE=%.5f",
              i, nrow(gridB), ss, cs, out$best_iter, out$test_mse))

  if (out$test_mse < best_mse_B) {
    best_mse_B <- out$test_mse
    best_cfg_B <- gridB[i, ]
    cat("  <-- NEW BEST")
  }
  cat("\n")
...
cat("\nTop 10 (Stage B):\n")
print(head(arrange(gridB, test_mse), 10))
cat("\nBest (Stage B):\n")
print(best_cfg_B)